...
It turns out that since Mood's median test is a type of chi-square test, we can express the effect sizes for Mood's median test the same way we can express the effect sizes for chi-square tests.

One way to express the effect size for a chi-square test on a $2 \times 2$ table is by computing the Phi coefficient $\varphi = \sqrt{\frac{\chi^2}{n}}$ where $\chi^2$ is the test statistic and $n$ is the total number of samples in the table.
