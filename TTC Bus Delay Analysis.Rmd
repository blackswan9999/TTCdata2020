...
```{r Eglinton Station, include=FALSE}
...
```{r Finch and Finch West, include=FALSE}
...
ttc_bus_delays_2020_all$Location = str_replace_all(ttc_bus_delays_2020_all$Location,  "YORKMILLS", "YORK MILLS")
...
There were several instances where Incidents were entered inconsistently. For example, there were instances where the incident description was entered as "Road Blocked - NON-TTC Collision", "Road Block - Non-TTC Collision", "Roadblock by Collision - Non-TTC". These were combined into one group with incident description "Collision - Non-TTC".
...
I made another csv file containing the names of Routes, the service type and the travel direction. This information was taken from the TTC website's description of Routes and Schedules. I joined this additional information with the TTC's dataset. Here is what the additional information looks like.
...
So far, the buses topping the list in terms of number of delays seem to be regular buses rather than express buses. Let's see if regular buses had longer delays than express buses. The TTC also runs community and night buses but not as frequently as the regular or express buses. So we'll focus on comparing the delay times of regular and express buses.
...
ttc_bus_delays_types %>% ggplot(aes(x=Service_Type, y =Delay_Time)) + geom_boxplot() + ggtitle("Delay Time for Regular and Express Buses")
```

So far, it seems that the mean delay time for regular buses is larger than the mean delay times of the express buses. To determine the best statistical test, we'll use Normal Quantile-Quantile plots to determine if the delay times are normally distributed.
...
Since the points on the Normal QQ Plots do not follow a straight line, the Regular and Express buses' Delay Times are skewed and do not follow a normal distribution. Since the data is not normally distributed, we will not use t-tests. Mood's Median Test is more suitable, as it does not require the populations to be normally distributed. We will use Mood's Median Test to see if the median Delay Time for the Regular buses is significantly higher than the median Delay Time for the Express buses. We'll test the null hypothesis that the median Delay time for Regular buses is equal to the median Delay time for Express buses.

First we'll find the median across both Express and Regular groups combined. Then we will see how many Regular bus delay times are greater than this median, and how many Express bus delay times are greater than this median. This gives us the following table.
...
We now run a two-sided Pearson chi-square test for testing the association between bus service type and whether delay times are higher than the median for both groups.
```{r echo=FALSE}
# Two Sided Chi-squared Test testing for association between median of Regular bus Delay Times, Express bus Delay Times
chisq_types <- chisq.test(types_table, correct = F)
chisq_types
```
...
## Experiment with Chebyshev's bound for Confidence Intervals
We could try using Chebyshev's bound to come up with confidence intervals for the Regular bus and Express bus delay times. Chebyshev's bound states that $1 - \frac{1}{k^2}$ percent of the distribution's values are within $k$ standard deviations of the mean. By Chebyshev's bound, 96% of a distribution's values are 5 standard deviations away from the mean. So a 96% confidence interval would correspond to $(\bar{x} - 5s, \bar{x} + 5s)$ where $\bar{x}$ is the mean and $s$ is the standard deviation. 

```{r echo=FALSE}
ttc_bus_delays_types %>% filter(Service_Type == "Regular") %>%
  select(Delay_Time) %>%
  summarize(Regular_Mean = mean(Delay_Time), Regular_SD = sd(Delay_Time), CI_Regular = str_c("( ", mean(Delay_Time)-5*sd(Delay_Time), " , ", mean(Delay_Time)+5*sd(Delay_Time), " )"))

ttc_bus_delays_types %>% filter(Service_Type == "Express") %>%
  select(Delay_Time) %>%
  summarize(Express_Mean = mean(Delay_Time), Express_SD = sd(Delay_Time), CI_Express = str_c("( ", mean(Delay_Time)-5*sd(Delay_Time), " , ", mean(Delay_Time)+5*sd(Delay_Time), " )"))
```

We can see that the 96% confidence intervals generated by Chebyshev's rule are quite large. When using Chebyshev's rule to construct 95% confidence intervals, we need to include data up to 4.5 standard deviations away from the mean, but when the data is normal we can just include 2 standard deviations instead of 4.5. So Chebyshev's bounds are less efficient compared to Gaussian bounds. Chebyshev's bound gives very large estimates but it can be somewhat useful for instances where we can't assume normality of the data.

## Effect Size

We have shown that there is a significant difference between the median delay times for Regular and Express buses. I would like to estimate the effect size, i.e. how much difference there is between the delay times of the Regular and Express buses. Usually Cohen's d is used to express the effect size when the data is normal. However as we have seen from the Normal QQ plots, the bus delay data is not normal.

It turns out that since Mood's median test is a type of chi-square test, we can express the effect sizes for Mood's median test the same way we can express the effect sizes for chi-square tests. One way to express the effect size for a chi-square test on a $2 \times 2$ table is by computing the Phi coefficient $\varphi = \sqrt{\frac{\chi^2}{n}}$ where $\chi^2$ is the test statistic and $n$ is the total number of samples in the table.

```{r echo=FALSE}
# number of samples
nrow_bustypes <- nrow(ttc_bus_delays_types)
print("n")
nrow_bustypes
# Chi-squared Test Statistic testing for association between bus service type, whether delay time is higher than grand median
chisq_stat_types <- chisq_types$statistic
print("Chi-squared test statistic")
chisq_stat_types

# Phi coefficient for effect size of chi-square test
phi_chisq_types <- sqrt(chisq_stat_types/nrow_bustypes)
print("Phi coefficient for effect size of chi-square test")
phi_chisq_types
```

We can see that $\varphi \approx 0.1341457$. We interpret $\varphi = 0.1$ as having a small effect, $\varphi = 0.3$ as having a medium effect, and $\varphi = 0.5$ as having a large effect, in accordance with Cohen's 1988 book *Statistical Power Analysis for the Behavioral Sciences* (see pages 224-227). Using this scale, bus service type (Regular/Express) has a small effect on delay time being greater than the grand median.   

Another way to express the effect size for a chi-square test on a $2 \times 2$ table is the odds ratio.

```{r, echo=FALSE}
library(epitools)
or_types <- oddsratio(types_table)
or_types
```

We see that the odds of a regular bus having a delay time greater than the median is nearly 2.98 times the odds of an express bus having a delay time greater than the median.

# Delays in Eastbound/Westbound buses vs Northbound/Southbound buses
In the past, I have heard Toronto residents talk about how the public transit system in Toronto is designed to bring people into the downtown core in the south of the city (as evidenced by the subway map), and how it is better for North to South travel rather than East to West travel. Thus I am curious to see if Eastbound/Westbound buses usually experience longer delays than Northbound/Southbound buses.

```{r echo=FALSE, fig.height=10, fig.width=8}
ttc_bus_delays_2020_all %>% ggplot(aes(x=Route_Direction, y =Delay_Time)) + geom_boxplot() + ggtitle("Delay Time for Eastbound/Westbound and Northbound/Southbound buses")
```
...
The East/West and North/South buses' Delay Times do not follow normal distributions, because the points on the Normal QQ Plots do not follow straight lines. Since the data is not normally distributed, we will not use a t-test to compare the two groups. We'll use Mood's Median Test since that doesn't require the delay times to be normally distributed. We'll test the null hypothesis that the median Delay time for East/West buses is equal to the median Delay time for North/South buses. We will see if the median Delay Time for the East/West buses is significantly higher than the median Delay Time for the North/South buses. 

First we'll find the median across both Eastbound/Westbound and Northbound/Southbound groups combined. Then we will see how many Eastbound/Westbound bus delay times are greater than this median, and how many Northbound/Southbound bus delay times are greater than this median. This gives us the following table.
...
We now run a Pearson chi-square test for testing the association between bus travel direction and delay time being greater than the median.

```{r echo=FALSE}
# Two Sided Chi-squared Test testing for association between median of E/W bus Delay Times, N/S bus Delay Times
(chisq_dir <- chisq.test(dir_table, correct = F))
```
...
We can express the effect size of the Mood's median test using the Phi coefficient or the odds ratio, since Mood's median test is a form of chi-square test.

```{r echo=FALSE}
# number of samples
nrow_busdir <- nrow(ttc_bus_delays_2020_all)
print("n")
nrow_busdir
# Chi-squared Test Statistic testing for association between bus direction, whether delay time is higher than grand median
chisq_stat_dir <- chisq_dir$statistic
print("Chi-squared test statistic")
chisq_stat_dir

# Phi coefficient for effect size of chi-square test
phi_chisq_dir <- sqrt(chisq_stat_dir/nrow_busdir)
print("Phi coefficient for effect size of chi-square test")
phi_chisq_dir
```

We see that the Phi coefficient is around 0.1176575. We interpret $\varphi = 0.1$ as having a small effect, $\varphi = 0.3$ as having a medium effect, and $\varphi = 0.5$ as having a large effect, in accordance with Cohen's 1988 book *Statistical Power Analysis for the Behavioral Sciences*. Using this scale, bus route direction has a small effect on delay time being greater than the grand median.   

We will now find the odds ratio.

```{r, echo=FALSE}
or_dir <- oddsratio(dir_table)
or_dir
```

We see that the odds of an East/West bus having a delay time greater than the overall median is around 1.606718 times the odds of a North/South bus having a delay time greater than the overall median. 

So there is a significant difference between the median delay times of North/South and East/West buses, but it is rather small.
...
Here we will see if there indeed is a statistically significant difference between the delay times for collisions involving TTC vehicles, and collisions not involving TTC vehicles. 
...
We can see that the standard deviation for the Collision - TTC group seems to be smaller than the one for the Collision - Non-TTC group. To determine what statistical test would be best for comparing the two groups, we will first test the normality of the data and see if the standard deviations of the two groups are the same.
...
The delay time is not normally distributed among the Collision - Non-TTC group because the points on the Normal QQ Plot don't follow a straight line.  

We will not use a t-test since the data is not normally distributed and the two groups do not have the same standard deviations. We will instead use Mood's Median Test to see if the median Delay Time for the Non-TTC Collision group is significantly higher than the median Delay Time for the TTC Collision group. We'll test the null hypothesis that the median Delay time for Regular buses is equal to the median Delay time for Express buses.

First we'll find the median across both Collision - TTC and Collision - Non-TTC groups combined. Then we will see how many TTC Collision delay times are greater than this median, and how many Non-TTC Collision delay times are greater than this median. This gives us the following table.
...
collision_table <- collision_table[2:1,]
collision_table
```
...
We now run a two-sided Pearson chi-square test for testing the association between Collision type and Delay Time being greater than overall median.
...
(chisq_col <- chisq.test(collision_table, correct = F))
```
...
```{r echo=FALSE}
# number of samples
nrow_buscol <- nrow(ttc_bus_delays_collisions)
print("n")
nrow_buscol
# Chi-squared Test Statistic testing for association between collision type, whether delay time is higher than grand median
chisq_stat_col <- chisq_col$statistic
print("Chi-squared test statistic")
chisq_stat_col

# Phi coefficient for effect size of chi-square test
phi_chisq_col <- sqrt(chisq_stat_col/nrow_buscol)
print("Phi coefficient for effect size of chi-square test")
phi_chisq_col
```

We see that the Phi coefficient is around 0.6374381. We interpret $\varphi = 0.1$ as having a small effect, $\varphi = 0.3$ as having a medium effect, and $\varphi = 0.5$ as having a large effect, in accordance with Cohen's 1988 book *Statistical Power Analysis for the Behavioral Sciences*. Using this scale, whether the collision involves TTC vehicles or not has a large effect on delay time being greater than the grand median.   

We will now find the odds ratio.

```{r, echo=FALSE}
or_col <- oddsratio(collision_table)
or_col
```

We see that the odds of an bus delayed due to a Non-TTC Collision having a delay time greater than the overall median is around 20.77161 times the odds of a bus delayed due to a TTC Collision having a delay time greater than the overall median. 

So there is a significant difference between the median delay times of North/South and East/West buses, and the difference is large.
...
* The median delay time for Regular buses was significantly higher than that of Express buses. Effect of bus service type on delay time being greater than median was small.
* The median delay time for East/West buses was significantly higher than that of North/South buses. Effect of route direction on delay time being greater than median was small.
* The median delay time for Non-TTC Collisions is significantly higher than that of TTC Collisions. Effect of collision type on delay time being greater than median was large.

# Work In Progress and Ideas for Further Avenues for Exploration
...
## Factors Influencing Delays at Subway Stations
So far, it seems that subway stations are the locations with the most delays. It would be interesting to gather additional data about these subway stations, and see why certain subway stations have more delays than others. Candidates to study include number of bus or streetcar routes connecting to the station, line number of subway, number of visitors per hour, and so on. 

## When Do Most Delays Happen?
It'd be interesting to see what times, days and seasons experience the most delays. It would be interesting to download weather data and match it to the incidents by date and time. Then it may be interesting to see if low temperatures and/or high precipitation may be correlated with more mechanical repairs, collisions or diversions.
...
I would like to create a model for predicting delay time. I would consider different regression models, feature selection on training/test sets using LASSO, and possibly more to see what variables are most predictive of delay time. Possible candidates include Direction, Location (eg. is it subway station, is it garage), frequency of service, Incident type, and more.

## Modelling Number of Delays Per Day as a Time Series
It may be interesting to model the number of delays per day as a time series, and do any necessary processing to ensure it is stationary. It may be interesting to try using the time series to predict how many delays there will be on a given day in the future. However, this will probably require processing and including the data over many years, not just the year 2020, to show any seasonal effects related to month.
...
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2850938/

https://www.statology.org/effect-size-chi-square/ 

https://www.rdocumentation.org/packages/epitools/versions/0.5-10.1/topics/oddsratio 
